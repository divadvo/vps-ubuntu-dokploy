...
```bash
curl -sSL https://raw.githubusercontent.com/alexandreravelli/vps-ubuntu-24-04-hardening-dokploy/release-1.0.2/setup.sh -o setup.sh && chmod +x setup.sh && ./setup.sh
```
...
| Supply chain | GPG fingerprint verification for Charm and Docker repos, scripts pinned to release tag (`release-1.0.2`) instead of `main` |
