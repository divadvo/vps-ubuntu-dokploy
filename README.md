...
```bash
curl -sSL https://raw.githubusercontent.com/alexandreravelli/vps-ubuntu-24-04-hardening-dokploy/release-1.0.15/setup.sh -o setup.sh && chmod +x setup.sh && ./setup.sh
```
...
| Supply chain | Charm and Docker repositories use GPG fingerprint verification; project scripts are pinned to release tag (`release-1.0.15`) instead of `main` |
